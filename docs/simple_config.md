...
`/data/local/tmp/JsxposedXSo/target_packages` is a simple text file
containing all the package names you want to inject frida into.

It accepts one package name per line f.e.
```
adb shell 'su -c "echo com.example.package > /data/local/tmp/JsxposedXSo/target_packages"'
```
...
`/data/local/tmp/JsxposedXSo/target_packages` accepts a start up delay in milliseconds.
You can provide it separated by a comma from the package_name.

f.e.
```
adb shell 'su -c "echo com.example.package,20000 > /data/local/tmp/JsxposedXSo/target_packages"'
```
...
The bundled gadget is located at `/data/local/tmp/JsxposedXSo/libgadget.so`.\
...
For this you can add the file `/data/local/tmp/JsxposedXSo/injected_libraries`.\
...
```
/data/local/tmp/JsxposedXSo/libhelperexample.so
/data/local/tmp/JsxposedXSo/libgadget.so
```
